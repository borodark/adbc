...
public:
    ...
    /// @return Status code
    AdbcStatusCode Connect(const std::string& host, int port, AdbcError* error = nullptr);
...
private:
    ...
    /// @param error Optional error output
    /// @return Status code
    AdbcStatusCode WriteMessage(const std::vector<uint8_t>& data, AdbcError* error = nullptr);
    ...
    /// @param error Optional error output
    /// @return Status code
    AdbcStatusCode ReadExact(uint8_t* buffer, size_t length, AdbcError* error = nullptr);
    ...
    /// @param error Optional error output
    /// @return Status code
    AdbcStatusCode WriteExact(const uint8_t* buffer, size_t length, AdbcError* error = nullptr);
