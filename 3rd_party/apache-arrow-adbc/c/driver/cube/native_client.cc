...
AdbcStatusCode NativeClient::Connect(const std::string& host, int port, AdbcError* error) {
    ...
    if (socket_fd_ < 0) {
        SetNativeClientError(error, "Failed to create socket: " + std::string(strerror(errno)));
    ...
    if (connect(socket_fd_, reinterpret_cast<struct sockaddr*>(&server_addr),
        ...
        close(socket_fd_);
        socket_fd_ = -1;
        SetNativeClientError(error, "Failed to connect to " + host + ":" + std::to_string(port) +
...
AdbcStatusCode NativeClient::PerformHandshake(AdbcError* error) {
    ...
    try {
        ...
        if (response->version != PROTOCOL_VERSION) {
            SetNativeClientError(error, "Protocol version mismatch. Client: " +
        ...
        SetNativeClientError(error, "Failed to decode handshake response: " + std::string(e.what()));
        return ADBC_STATUS_INVALID_DATA;
    }
...
AdbcStatusCode NativeClient::Authenticate(const std::string& token,
    ...
    try {
        ...
        SetNativeClientError(error, "Failed to decode authentication response: " + std::string(e.what()));
        return ADBC_STATUS_INVALID_DATA;
    }
    ...
    while (!query_complete) {
        ...
        try {
            switch (msg_type) {
                ...
                default: {
                    SetNativeClientError(error, "Unexpected message type: " +
                                        std::to_string(static_cast<uint8_t>(msg_type)));
                    return ADBC_STATUS_INVALID_DATA;
                }
            }
        } catch (const std::exception& e) {
            SetNativeClientError(error, "Failed to decode response: " + std::string(e.what()));
            return ADBC_STATUS_INVALID_DATA;
        }
    }
...
std::vector<uint8_t> NativeClient::ReadMessage(AdbcError* error) {
    ...
    if (length == 0 || length > 100 * 1024 * 1024) {  // 100MB max
        SetNativeClientError(error, "Invalid message length: " + std::to_string(length));
        return {};
    }
...
AdbcStatusCode NativeClient::ReadExact(uint8_t* buffer, size_t length, AdbcError* error) {
    ...
    while (total_read < length) {
        ...
        if (n < 0) {
            ...
            SetNativeClientError(error, "Socket read error: " + std::string(strerror(errno)));
            return ADBC_STATUS_IO;
        }
        if (n == 0) {
...
AdbcStatusCode NativeClient::WriteExact(const uint8_t* buffer, size_t length, AdbcError* error) {
    ...
    while (total_written < length) {
        ...
        if (n < 0) {
            if (errno == EINTR) continue;  // Interrupted, retry
            SetNativeClientError(error, "Socket write error: " + std::string(strerror(errno)));
            return ADBC_STATUS_IO;
        }
        total_written += n;
    }
