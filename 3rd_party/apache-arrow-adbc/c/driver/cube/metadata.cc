...
struct ArrowSchema MetadataBuilder::Build() {
  ...
  schema.children =
      static_cast<struct ArrowSchema**>(malloc(schema.n_children * sizeof(struct ArrowSchema*)));

  // Build each child field
  for (int64_t i = 0; i < schema.n_children; i++) {
    schema.children[i] = static_cast<struct ArrowSchema*>(malloc(sizeof(struct ArrowSchema)));
    ...
    ArrowType arrow_type = CubeTypeMapper::MapCubeTypeToArrowType(column_types_[i]);
    ...
    child->metadata = nullptr;  // Simplified: would store type mapping in metadata
  }
