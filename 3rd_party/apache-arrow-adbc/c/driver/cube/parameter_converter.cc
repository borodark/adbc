...
std::string FormatTimestamp(int64_t micros_since_epoch) {
  ...
  oss << std::setfill('0') << std::setw(4) << (tm_result.tm_year + 1900)
      << "-" << std::setw(2) << (tm_result.tm_mon + 1) << "-"
      << std::setw(2) << tm_result.tm_mday << "T" << std::setw(2)
      << tm_result.tm_hour << ":" << std::setw(2) << tm_result.tm_min << ":"
      << std::setw(2) << tm_result.tm_sec << "." << std::setw(6) << micros;
  return oss.str();
}
...
std::string ParameterConverter::ConvertBinary(const ArrowArray* array,
  ...
  for (int32_t i = 0; i < length; i++) {
    oss << std::setfill('0') << std::setw(2) << std::hex << data[start_offset + i];
  }
  return oss.str();
}
...
std::vector<std::string> ParameterConverter::ConvertArrowArrayToParams(
...
const char** ParameterConverter::GetParamValuesCArray(
  ...
  char** result = static_cast<char**>(malloc(param_values.size() * sizeof(char*)));
