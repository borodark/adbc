...
void MessageCodec::PutString(std::vector<uint8_t>& buf, const std::string& str) {
    PutU32(buf, static_cast<uint32_t>(str.length()));
    buf.insert(buf.end(), str.begin(), str.end());
}

void MessageCodec::PutOptionalString(std::vector<uint8_t>& buf, const std::string& str) {
    if (!str.empty()) {
        PutU8(buf, 1);
        PutString(buf, str);
    } else {
...
void MessageCodec::PutBytes(std::vector<uint8_t>& buf, const std::vector<uint8_t>& bytes) {
    PutU32(buf, static_cast<uint32_t>(bytes.size()));
    buf.insert(buf.end(), bytes.begin(), bytes.end());
}

uint32_t MessageCodec::GetU32(const uint8_t*& ptr, const uint8_t* end) {
    if (ptr + 4 > end) throw std::runtime_error("Insufficient data for U32");
    uint32_t net_value;
...
int64_t MessageCodec::GetI64(const uint8_t*& ptr, const uint8_t* end) {
    if (ptr + 8 > end) throw std::runtime_error("Insufficient data for I64");
    uint64_t value = 0;
    for (int i = 0; i < 8; ++i) {
        value = (value << 8) | ptr[i];
    }
...
uint8_t MessageCodec::GetU8(const uint8_t*& ptr, const uint8_t* end) {
    if (ptr >= end) throw std::runtime_error("Insufficient data for U8");
    return *ptr++;
}

std::string MessageCodec::GetString(const uint8_t*& ptr, const uint8_t* end) {
    uint32_t length = GetU32(ptr, end);
    if (ptr + length > end) throw std::runtime_error("Insufficient data for string");
    std::string result(reinterpret_cast<const char*>(ptr), length);
    ptr += length;
    return result;
}

std::string MessageCodec::GetOptionalString(const uint8_t*& ptr, const uint8_t* end) {
...
std::vector<uint8_t> MessageCodec::GetBytes(const uint8_t*& ptr, const uint8_t* end) {
    uint32_t length = GetU32(ptr, end);
    if (ptr + length > end) throw std::runtime_error("Insufficient data for bytes");
...
std::unique_ptr<HandshakeResponse> HandshakeResponse::Decode(const uint8_t* data, size_t length) {
...
std::unique_ptr<AuthResponse> AuthResponse::Decode(const uint8_t* data, size_t length) {
...
std::unique_ptr<QueryResponseSchema> QueryResponseSchema::Decode(const uint8_t* data, size_t length) {
...
std::unique_ptr<QueryResponseBatch> QueryResponseBatch::Decode(const uint8_t* data, size_t length) {
...
std::unique_ptr<QueryComplete> QueryComplete::Decode(const uint8_t* data, size_t length) {
...
std::unique_ptr<ErrorMessage> ErrorMessage::Decode(const uint8_t* data, size_t length) {
